//two wires start from the same point and intersect
//find the distance at where they intersect closest from the starting port
// |x1 - x2| + |y1 - y2|
//init storage of all intersection 
//loop through first line and store all points in hash map
//loop through second line and compare points to hash map


function findClosestIntersection(str1, str2) {
  let arr1 = str1.split(',')
  let arr2 = str2.split(',')
  let firstPoint = [0, 0]
  let secPoint = [0, 0]
  ...
  for (let i = 0; i < arr1.length; i++) {
    let dir = arr1[i]
    if (dir[0] === 'R') firstPoint[0] += Number(dir.slice(1))
    if (dir[0] === 'L') firstPoint[0] -= Number(dir.slice(1))
    if (dir[0] === 'U') firstPoint[1] += Number(dir.slice(1))
    if (dir[0] === 'D') firstPoint[1] -= Number(dir.slice(1))
    firstPointsRec.push([firstPoint[0], firstPoint[1]])
  }
  ...
  for(let j = 0; j < arr2.length; j++) {
    let dir2 = arr2[j]
    if (dir2[0] === 'R') secPoint[0] += Number(dir2.slice(1))
    if (dir2[0] === 'L') secPoint[0] -= Number(dir2.slice(1))
    if (dir2[0] === 'U') secPoint[1] += Number(dir2.slice(1))
    if (dir2[0] === 'D') secPoint[1] -= Number(dir2.slice(1))
    if (firstPointsRec.includes([secPoint[0], secPoint[1]])) {
      intersections.push([secPoint[0], secPoint[1]])
    }
  }
